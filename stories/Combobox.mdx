...
<Meta title="Documentation/Forms/Combobox" />
...
### When to Use
- ✅ Searchable dropdowns with 8+ options
- ✅ Autocomplete and type-ahead scenarios
- ✅ Long lists that benefit from filtering (countries, products)
- ✅ When users might not know exact option name
- ✅ Forms where search improves efficiency

### When NOT to Use
- ❌ Short lists (< 8 options - use Select or RadioGroup)
- ❌ Free text entry without options (use Input)
- ❌ Binary choices (use Switch or Checkbox)
- ❌ Multiple simultaneous selections (use multi-select variant)
- ❌ When browsing all options is important (use Select)
