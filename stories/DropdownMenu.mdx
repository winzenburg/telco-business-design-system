...
<Meta title="Documentation/DropdownMenu" />
...
With Checkboxes:
    ...
    │ View Settings           │
    ├─────────────────────────┤
    │ ☑ Show Toolbar          │ ← Checked item
    │ ☐ Show Sidebar          │ ← Unchecked item
    │ ☑ Show Status Bar       │
    └─────────────────────────┘
...
### When to Use
- ✅ Multiple related actions on a single element
- ✅ Account and user profile menus
- ✅ Contextual actions for table rows or cards
- ✅ View options (sort, filter, display settings)
- ✅ Action overflow in toolbars (more actions)
- ✅ Mobile navigation menu collapsing
- ✅ Status or category selection

### When NOT to Use
- ❌ Primary navigation (use Navigation component)
- ❌ Single action (use Button)
- ❌ Rich interactive content (use Popover or Dialog)
- ❌ Long lists (over 15 items) - use Select or Combobox
- ❌ Forms with multiple inputs (use Form or Dialog)
- ❌ Large hierarchical menus (use Navigation tree)
...
**Example:** Share → (Copy Link, Download, Email)
...
- Focus loops: Down from last item → first item
...
```
My Account           ← Label
├─ Profile          ← Account actions group
├─ Settings
├─ Billing
──────────────       ← Separator
Quick Actions        ← Label
├─ Download report   ← Action group
├─ Share dashboard
──────────────       ← Separator
Log out             ← Destructive action, isolated
```
...
├─ Profile
├─ Log out          ← Destructive action not isolated
├─ Settings
├─ Download report
├─ Billing
├─ Share dashboard
...
**When to Choose:**
- **DropdownMenu** → Contextual actions, account menus, row actions
- **Select** → Form inputs, single selection from options
- **Combobox** → Searchable selection, large option lists
- **Popover** → Rich content, forms, non-action overlays
- **Sheet** → Settings panels, filters, large action sets
- **Navigation Menu** → Primary site navigation, multi-level menus
